    ...
    /* [out] */ ULONG __RPC_FAR *pcbRead)
{
    ...
    hr = E_FAIL;
    PyWinObject_GetBuffer pybuf(result);
    if (pybuf.ok()) {
        if (pybuf.len() > cb)
    ...
    /* [out] */ ULARGE_INTEGER __RPC_FAR *pcbWritten)
{
    ...
    if (!PyArg_ParseTuple(result, "OO", &obpcbRead, &obpcbWritten))
        return PyCom_HandlePythonFailureToCOM(/*pexcepinfo*/);
    BOOL bPythonIsHappy = TRUE;
    if (pcbRead && !PyWinObject_AsULARGE_INTEGER(obpcbRead, pcbRead))
        bPythonIsHappy = FALSE;
    if (pcbWritten && !PyWinObject_AsULARGE_INTEGER(obpcbWritten, pcbWritten))
        bPythonIsHappy = FALSE;
    if (!bPythonIsHappy)
        hr = MAKE_PYCOM_GATEWAY_FAILURE_CODE("CopyTo");
    Py_DECREF(result);
    return hr;
}
...
STDMETHODIMP PyGStream::Revert(void)
{
    PY_GATEWAY_METHOD;
    ...
    /* [out] */ IStream __RPC_FAR *__RPC_FAR *ppstm)
{
    ...
    if (!PyCom_InterfaceFromPyObject(obppstm, IID_IStream, (void **)ppstm, FALSE /* bNoneOK */))
        bPythonIsHappy = FALSE;
    if (!bPythonIsHappy)
        hr = MAKE_PYCOM_GATEWAY_FAILURE_CODE("Clone");
    Py_DECREF(result);
    return hr;
}
