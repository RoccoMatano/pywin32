...
static PyObject *PyWin_NewUnicodeFromRaw(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "O", &ob))
        return NULL;
    PyWinObject_GetBuffer pybuf(ob);
    if (!pybuf.ok())
        return NULL;
...
PyWinObject_GetBuffer::PyWinObject_GetBuffer()
{
    memset(&m_view, 0, sizeof(m_view));
}

bool PyWinObject_GetBuffer::init(PyObject *ob, bool bWrite, bool bNoneOk)
{
    release();
    memset(&m_view, 0, sizeof(m_view));
    if (ob == Py_None) {
...
PyWinObject_GetBuffer::PyWinObject_GetBuffer(PyObject *ob, bool bWrite, bool bNoneOk)
{
    memset(&m_view, 0, sizeof(m_view));
    init(ob, bWrite, bNoneOk);
}

void PyWinObject_GetBuffer::release()
{
...
bool PyWinObject_GetBuffer::ok()
{
    return m_view.obj != NULL;
}

void* PyWinObject_GetBuffer::ptr()
{
    return m_view.buf;
}

DWORD PyWinObject_GetBuffer::len()
{
