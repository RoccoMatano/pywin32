...
static PyObject *PySetMemory(PyObject *self, PyObject *args)
{
	...
	if (!PyWinLong_AsVoidPtr(obaddr, &addr))
		return NULL;
    PyWinObject_GetBuffer pybuf(obsrc);
	if (!pybuf.ok())
	    return NULL;
...
static PyObject *PyCreateIconFromResource(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "Oi|i", &obbits, &isIcon, &ver))
		return NULL;
    PyWinObject_GetBuffer pybuf(obbits);		
    if (!pybuf.ok())
		return NULL;
...
static PyObject *PyInsertMenuItem(PyObject *self, PyObject *args)
{
    ...
    PyWinObject_GetBuffer pybuf(info_obj);
    if (!pybuf.ok())
        return NULL;
...
static PyObject *PyGetMenuItemInfo(PyObject *self, PyObject *args)
{
    ...
    PyWinObject_GetBuffer pybuf(info_obj, true);
    if (!pybuf.ok())
        return NULL;
	if (sizeof(MENUITEMINFO) != pybuf.len())
		return PyErr_Format(PyExc_TypeError,
...
PyObject *PySetMenuInfo(PyObject *self, PyObject *args)
{
    ...
    PyWinObject_GetBuffer pybuf(obInfo);
	if (!pybuf.ok())
		return NULL;
...
PyObject *PyGetMenuInfo(PyObject *self, PyObject *args)
{
    ...
    PyWinObject_GetBuffer pybuf(obInfo, true);
	if (!pybuf.ok())
		return NULL;
	if (sizeof(MENUINFO) != pybuf.len())
...
PyObject *PyRegisterDeviceNotification(PyObject *self, PyObject *args)
{
	...
	if (!PyWinObject_AsHANDLE(obh, &handle))
		return NULL;
    PyWinObject_GetBuffer pybuf(obFilter);
	if (!pybuf.ok())
		return NULL;
    // basic sanity check.
