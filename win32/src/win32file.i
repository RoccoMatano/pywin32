...
PyObject *py_DeviceIoControl(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyWinObject_AsHANDLE(obhFile, &hDevice))
		return NULL;
    PyWinObject_GetBuffer out_buf, in_buf(obInBuffer, false, true); // None Ok
	if (!in_buf.ok())
		return NULL;
...
PyObject *MyWriteFile(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "OO|O:WriteFile", 
		...
		return NULL;
    PyWinObject_GetBuffer pybuf(obWriteData);
    if (!pybuf.ok())
        return NULL;
...
static PyObject *py_TransmitFile( PyObject *self, PyObject *args, PyObject *kwargs ) {
	...
	PyWinObject_GetBuffer head(obHead, false, true);
	if (!head.ok())
		return NULL;
	tf_buffers.Head = head.ptr();
	tf_buffers.HeadLength = head.len();
	PyWinObject_GetBuffer tail(obTail, false, true);
	if (!tail.ok())
		return NULL;
...
static PyObject *py_ConnectEx( PyObject *self, PyObject *args, PyObject *kwargs ) {
	...
	PyWinObject_GetBuffer pybuf(obBuf, false, true); // None Ok
	if (!pybuf.ok())
		return NULL;
...
)
{
	...
	PyWinObject_GetBuffer pybuf(obBuf, true);
	if (!pybuf.ok())
		return NULL;
